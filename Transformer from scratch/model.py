...
class DecoderBlock(nn.Module):
    ...
    def forward(self, x, encoder_output, src_mask, tgt_mask): 
        ...
        x = self.residual_connections[1](x, encoder_output, encoder_output, src_mask)
...
class ProjectionLayer(nn.Module):
    ...
    def __init__(self, d_model: init, vocab_size: int) -> None:
        super().__init__()
...
class Transformer(nn.Module):
    ...
    def __init__(self, 
                encoder: Encoder, 
                decoder: Decoder, 
                src_embded: InputEmbeddings,
                tgt_embeded: InputEmbeddings,
                src_pos: PositionalEnconding,
                tgt_pos: PositionalEnconding,
    ...
    def encode(self, src, src_mask):
        src = self.src_embded(src)
        src = self.src_pos(src)
        return self.encoder(src, src_mask)

    def decode(self, encoder_output, src_mask, tgt, tgt_mask):
        tgt = self.tgt_embeded(tgt)
